---
name: treeos-setup-production-local-amd
...
**DO NOT** copy or create any setup scripts. The production setup script already exists in the repository at `skills/treeos-setup-production-local-amd/run.sh`. Always reference and use this existing script. Never create duplicates or copies.
...
NOTE: The run.sh script is already marked as executable in the repository (chmod +x is committed in git).
...
!echo "sudo ./skills/treeos-setup-production-local-amd/run.sh"
!echo ""
