---
name: treeos-clean-production-data
description: Clean TreeOS production data while preserving shared folders, with sudo guidance and confirmation.
---
...
**To confirm deletion, please type "yes" when prompted.**
...
```bash
sudo ./skills/treeos-clean-production-data/run.sh
```

4. Offer to review the script for safety:
   - Ask if they'd like you to review the cleanup script first
   - If yes, read and analyze `skills/treeos-clean-production-data/run.sh`
...
!echo "sudo ./skills/treeos-clean-production-data/run.sh"
...
- Use the non-interactive script `skills/treeos-clean-production-data/run.sh` after confirmation
