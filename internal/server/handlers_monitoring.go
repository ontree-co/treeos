...
func (s *Server) handleMonitoringDetail(w http.ResponseWriter, r *http.Request) {
	...
	if cached, found := s.sparklineCache.Get(cacheKey); found {
		...
		// Get metrics batch from database
		batch, err := database.GetMetricsBatch(startTime, endTime)
		if err != nil {
			log.Printf("Failed to get metrics batch: %v", err)
			batch = &database.MetricsBatch{Metrics: []database.SystemVitalLog{}}
		...
		case "download":
			...
			for _, metric := range batch.Metrics {
				chartData.Points = append(chartData.Points, charts.DataPoint{
					Time:  metric.Timestamp,
					Value: float64(metric.DownloadRate) / 1024 / 1024,
				})
			}
		...
		case "upload":
			...
			for _, metric := range batch.Metrics {
				chartData.Points = append(chartData.Points, charts.DataPoint{
					Time:  metric.Timestamp,
					Value: float64(metric.UploadRate) / 1024 / 1024,
				})
			}
		...
		// Generate the detailed chart
		chartSVG = charts.GenerateDetailedChart(chartData, 900, 500)
	...
	data["MetricTitle"] = getMetricTitle(metricType)
	data["TimeRange"] = timeRange
	data["ChartSVG"] = chartSVG

	// Check if template exists, otherwise render inline HTML
	if tmpl, ok := s.templates["monitoring_detail"]; ok {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		if err := tmpl.ExecuteTemplate(w, "base", data); err != nil {
			log.Printf("Failed to execute monitoring detail template: %v", err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
		}
	} else {
		// Fallback: render inline HTML
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		html := fmt.Sprintf(`
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>%s - TreeOS Monitoring</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container-fluid">
            <a class="navbar-brand" href="/">TreeOS</a>
    ...
    <div class="container mt-4">
        <div class="card">
            ...
            <div class="card-body">
                <div class="btn-group mb-3" role="group" aria-label="Time range selector">
                    <a href="?range=1h" class="btn btn-sm %s">1 Hour</a>
                    <a href="?range=6h" class="btn btn-sm %s">6 Hours</a>
                    <a href="?range=24h" class="btn btn-sm %s">24 Hours</a>
                    <a href="?range=7d" class="btn btn-sm %s">7 Days</a>
                </div>
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
			...
			getMetricTitle(metricType),
			ifElse(timeRange == "1h", "btn-primary", "btn-outline-primary"),
			ifElse(timeRange == "6h", "btn-primary", "btn-outline-primary"),
			ifElse(timeRange == "24h", "btn-primary", "btn-outline-primary"),
			ifElse(timeRange == "7d", "btn-primary", "btn-outline-primary"),
			chartSVG,
		)
...
// getMetricTitle returns a human-readable title for a metric type
func getMetricTitle(metricType string) string {
	switch metricType {
	case "cpu":
		return "CPU Usage"
	case "memory":
