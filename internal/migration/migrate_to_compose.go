...
func MigrateDeployedAppsToCompose(cfg *config.Config) error {
	logging.Info("Starting migration of deployed_apps to docker-compose.yml files...")
	...
	logging.Infof("Created backup directory: %s", backupDir)
	...
	logging.Infof("Found %d apps to migrate", len(apps))

	if len(apps) == 0 {
		logging.Info("No apps found in deployed_apps table. Migration complete.")
		return nil
	}
	...
	for _, app := range apps {
		logging.Infof("Migrating app: %s", app.Name)
		...
		if _, err := os.Stat(appPath); os.IsNotExist(err) {
			logging.Warnf("WARNING: App directory not found for %s, skipping", app.Name)
			continue
		}

		// Check if compose file exists
		if _, err := os.Stat(composePath); os.IsNotExist(err) {
			logging.Warnf("WARNING: docker-compose.yml not found for %s, skipping", app.Name)
			continue
		}
		...
		if err := copyFile(composePath, backupPath); err != nil {
			logging.Errorf("ERROR: Failed to backup compose file for %s: %v", app.Name, err)
			continue
		}
		...
		if err != nil {
			logging.Errorf("ERROR: Failed to read compose file for %s: %v", app.Name, err)
			continue
		}
		...
		if err := yamlutil.WriteComposeWithMetadata(composePath, compose); err != nil {
			logging.Errorf("ERROR: Failed to write compose file for %s: %v", app.Name, err)
			continue
		}

		logging.Infof("✓ Successfully migrated app: %s (subdomain: %s, port: %d, exposed: %v)",
			app.Name, app.Subdomain, app.HostPort, app.IsExposed)
		successCount++
	}

	logging.Infof("Migration complete: %d/%d apps migrated successfully", successCount, len(apps))

	if successCount < len(apps) {
		logging.Warnf("WARNING: %d apps failed to migrate. Check logs above for details.", len(apps)-successCount)
		logging.Infof("Backup files are available in: %s", backupDir)
		return fmt.Errorf("migration completed with errors: %d/%d apps migrated", successCount, len(apps))
	}

	logging.Info("All apps migrated successfully!")
	logging.Infof("Backup files are available in: %s", backupDir)
	logging.Info("You can now safely remove the deployed_apps table from the database.")
...
func getAllDeployedApps() ([]DeployedApp, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			logging.Warnf("WARNING: Failed to scan row: %v", err)
			continue
		}
