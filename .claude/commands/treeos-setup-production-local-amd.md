---
description: Setup TreeOS on local AMD AI 395+ desktop with GPU support
argument-hint:
---

# Setup TreeOS Production Environment (Local AMD GPU)

## IMPORTANT AGENT INSTRUCTIONS
**DO NOT** copy or create any setup scripts. The production setup script already exists in the repository at `.claude/commands/treeos-setup-production-local-amd.sh`. Always reference and use this existing script. Never create duplicates or copies.

## Overview
This command will set up TreeOS in production mode on a local AMD AI 395+ desktop computer with full GPU support for AI inference. It installs AMD ROCm drivers and configures GPU permissions.
...
NOTE: The treeos-setup-production-local-amd.sh script is already marked as executable in the repository (chmod +x is committed in git).
...
!echo "sudo ./.claude/commands/treeos-setup-production-local-amd.sh"
!echo ""
