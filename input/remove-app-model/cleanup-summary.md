...
### 1. `/opt/ontree/ontree-node/internal/database/models.go`
- **Status**: ✅ ALREADY REMOVED - The DeployedApp struct has been removed from this file
- No further action needed

### 2. `/opt/ontree/ontree-node/internal/server/server.go`
...
### 3. `/opt/ontree/ontree-node/internal/server/handlers.go`
...
### 4. `/opt/ontree/ontree-node/internal/migration/migrate_to_compose.go`
...
- `/opt/ontree/ontree-node/migrations/002_add_deployed_apps.sql` - Creates the table
- `/opt/ontree/ontree-node/migrations/004_drop_deployed_apps.sql` - Drops the table

## Documentation Files (No action needed - these document the changes)
- Various CLAUDE.md files documenting the migration
- Input specification files in `/opt/ontree/ontree-node/input/`
