...
2.  **Introduce Cobra:** Add Cobra as a dependency. Create a `main.go` that initializes a root Cobra command (`ontree`).
3.  **Implement CLI Dispatching:**
    ...
    *   Create a placeholder `app` command in `cmd/app.go` (`ontree app start`) that prints "Starting app..." and exits.
...
**Acceptance Criteria:**
1.  **Define Core Types:** Create `internal/ontree/types.go`. Define the core structs (`App`, `Config`, `Status`, etc.) that represent your domain.
2.  **Create the Manager:** Create `internal/ontree/manager.go` with a `Manager` struct that accepts a `*slog.Logger`.
3.  **Migrate Logic:**
    *   Identify the functions that handle Podman interaction. Move them into `internal/ontree/podman.go` as un-exported helper functions.
    *   Identify Tailscale logic and move it to `internal/ontree/tailscale.go`.
    *   Create synchronous public methods on the `Manager` struct (e.g., `StartApp(...) (*Status, error)`, `StopApp(...) error`). These methods will orchestrate the calls to the helper functions.
...
1.  **Update CLI Commands (`cmd/`):**
    ...
    *   Call the synchronous manager methods (e.g., `manager.StartApp(...)`).
...
2.  **Implement WebSocket Handler:**
    ...
    *   It must upgrade the HTTP connection to a WebSocket.
    *   It calls the async `manager.StartApp`, passing the request's context.
    *   It ranges over the returned channel, marshals each event to JSON, and sends it as a message over the WebSocket.
